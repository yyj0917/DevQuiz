...
import type { SavedQuestionWithQuestion } from '@/types/database';

interface SavedQuestionsListProps {
  initialSavedQuestions: SavedQuestionWithQuestion[];
}

export function SavedQuestionsList({
  initialSavedQuestions,
  ...
  if (initialSavedQuestions.length === 0) {
    return (
  ...
  return (
    <div className="space-y-4">
      {initialSavedQuestions.map((item) => {
      ...
      <div className="text-center text-sm text-gray-600">
        총 {initialSavedQuestions.length}개의 문제를 저장했습니다
      </div>
    </div>
  );
}
