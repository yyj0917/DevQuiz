...
interface WrongNotesListProps {
  initialWrongNotes: WrongNoteWithQuestion[];
  initialFilter: 'all' | 'unreviewed' | 'reviewed';
}
...
export function WrongNotesList({
  ...
  const filter = (searchParams.get('filter') as 'all' | 'unreviewed' | 'reviewed') || initialFilter;
  ...
  const filteredNotes = filter === 'all'
    ? initialWrongNotes
    : filter === 'reviewed'
    ? initialWrongNotes.filter((note) => note.is_reviewed)
    : initialWrongNotes.filter((note) => !note.is_reviewed);
    ...
    <>
      ...
      {filteredNotes.length > 0 && (
        <div className="text-center text-sm text-gray-600">
          총 {filteredNotes.length}개의 오답이 있습니다
        </div>
      )}
